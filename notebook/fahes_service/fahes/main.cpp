...
// ================The main Function====================================
extern "C"
int execute(char * table_name, char * out_directory) {
    
    string file_name = string(table_name);
    string output_dir = string(out_directory);
    cerr << "Running FAHES on : " << file_name << " and the results will be stored in : " << output_dir << endl;
    // if (debug)
    ...
    cerr << "Data uploaded successfully ...\n";
    cerr << "The table has : " << T.data.size() << " columns and " << T.data[0].size() << " rows" << endl;
    F_DiMaC FDiMaC;
